...
def deposit(accountNumber, amountTransaction, location):
    if(amountTransaction > 0):
        current_account = local_session.query(Accounts.account_id, Accounts.balance).filter(
            Accounts.account_number == accountNumber).first()
        if(current_account != ""):
            local_session.add(Transactions(amount=amountTransaction, transaction_type_id=1,
                              account_id=current_account[0], bank_location_id=location))
            local_session.query(Accounts).filter(Accounts.account_id == current_account[0]).update(
                {'balance': current_account[1] + amountTransaction})
...
def withdraw(accountNumber, amountTransaction, location):
    if(amountTransaction > 0):
        ...
        if(current_account != "" and current_account[1] > amountTransaction):
            local_session.add(Transactions(amount=amountTransaction, transaction_type_id=2,
                              account_id=current_account[0], bank_location_id=location))
            local_session.query(Accounts).filter(Accounts.account_id == current_account[0]).update(
                {'balance': current_account[1] - amountTransaction})
            return "The transaction was completed successfully!"
...
print(deposit(12345678, 1000, 1))

most_transaction = local_session.query(BankLocations.city, BankLocations.bank_address, func.count(Transactions.bank_location_id).label(
    'count')).filter(Transactions.bank_location_id == BankLocations.bank_location_id).group_by(BankLocations.bank_location_id)
...
account_with_most_transaction = local_session.query(Accounts.account_name, Accounts.account_surname, func.max(
    Transactions.account_id)).filter(Transactions.account_id == Accounts.account_id)

for acc in account_with_most_transaction:
    print("Osoba koja je obavila najviše transakcija je:",
          acc.account_name, acc.account_surname)

print("---------------------------------- ")

average_deposit = local_session.query(func.avg(Transactions.amount).label(
    'average')).filter(Transactions.transaction_type_id == 1)
